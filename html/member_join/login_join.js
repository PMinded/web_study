function onLoad(){
    //패턴검색내용
    const idPattern=/^[\w]{3,}$/;   //[\w]는 영문자, 숫자, _만 입력 가능/{3,}은 3글자이상가능
    const pwdPattern=/^[\w]{6,10}$/;   //영문자와 숫자,_ 6~10
    const namePattern=/^[가-힣]{2,4}|[A-Z]{1}[a-zA-Z\x20]{1,19}$/;   //한글 2~4글자, 영문자 2~10글자 첫글자는 대문자 공백가능
    const nicknamePattern=/^[\w가-힣]{4,}$/;   //공백없이 한글,영문,숫자만 입력 가능(4글자 이상)
    const emailPattern=/^[a-z0-9_+.-]+@([a-z0-9-]+.)+[a-z0-9]{2,4}$/;
    const telPattern=/^[\d]{2,3}-[\d]{3,4}-[\d]{4}$/; // \d는 숫자만 가능
    const mobilePattern=/^010-(?:[\d]{3}|[\d]{4})-[\d]{4}$/; // \d는 숫자만 가능
    const datePattern=/^[\d]{4}-[\d]{2}-[\d]{2}$/; // \d는 숫자만 가능
    ...
    inputPW1.addEventListener("blur",()=>validate(inputPW1, pwdPattern, "영문자, 숫자, _ 6~10"));
    ...
    inputName.addEventListener("blur",()=>validate(inputName, namePattern, "한글 2~4글자, 영문자 2~10글자 첫글자는 대문자 공백가능"));
    inputNickname.addEventListener("blur",()=>validate(inputNickname, nicknamePattern, "공백없이 한글,영문,숫자만 입력 가능(4글자 이상)"));
    inputEmail.addEventListener("blur",()=>validate(inputEmail, emailPattern, "이메일 형식에 맞지 않습니다"));
    inputTel.addEventListener("blur",()=>validate(inputTel, telPattern, "전화번호 형식에 맞지 않습니다"));
    inputMobile.addEventListener("blur",()=>validate(inputMobile, mobilePattern, "핸드폰번호 형식에 맞지 않습니다"));
    ...
    myform.addEventListener("submit",(e)=>{
        ...
        validate(inputName, namePattern, "한글 2~4글자, 영문자 2~10글자 첫글자는 대문자 공백가능");
        validate(inputNickname, nicknamePattern, "공백없이 한글,영문,숫자만 입력 가능(4글자 이상)");
        validate(inputEmail, emailPattern, "이메일 형식에 맞지 않습니다");
        validate(inputTel, telPattern, "전화번호 형식에 맞지 않습니다");
        validate(inputMobile, mobilePattern, "핸드폰번호 형식에 맞지 않습니다");
        validate(inputDate, datePattern, "날짜를 선택해주세요");
        if(zipcode.value===""||addr1.value===""){
            zipcode.nextSibling.textContent="주소를 선택해주세요";
            zipcode.focus();
            return;
        }
        alert("서버로 전송하겠습니다");
